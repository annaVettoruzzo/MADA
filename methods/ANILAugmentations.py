...
# -------------------------------------------------------------------
class ANILAug:
    def __init__(self, model, loss_fn, lr_inner, nb_augs, lr_outer=0.001, adapt_steps=1, anil_last=False, with_weights=False):
        self.model = model
        ...
        if anil_last:
            self.theta = {name: value for name, value in dict(self.model.named_parameters()).items() if 'last' in name}
        else:
            self.theta = {name: value for name, value in dict(self.model.named_parameters()).items() if 'last' in name or 'dense_block2' in name}
        ...
        if self.with_weights:
            ...
            meta_params = list(self.model.parameters()) + [self.weights]
        else:
